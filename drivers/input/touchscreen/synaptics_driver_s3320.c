...
static ssize_t synaptics_rmi4_baseline_show(struct device *dev, char *buf,
					    bool savefile);
				  ...
				  const struct i2c_device_id *id)
{
	...
	for (i = 0; i < NR_CPUS; i++) {
		TPD_ERR("check CPU[%d] is [%s]\n", i,
			cpu_is_offline(i) ? "offline" : "online");
		if (cpu_online(i) && (i != smp_processor_id()))
			break;
	}
	queue_delayed_work_on(i != NR_CPUS ? i : 0, optimize_data.workqueue,
...
static int synaptics_read_register_map(struct synaptics_ts_data *ts)
{
	...
	if (ret < 0) {
		TPD_ERR
		    ("synaptics_read_register_map: failed for page select\n");
		return -1;
	}
						  ...
						  int enable)
{
	...
	ret =
	    i2c_smbus_write_i2c_block_data(ts->client, F12_2D_CTRL20, 3,
					   &(reportbuf[0x0]));
	if (ret < 0) {
...
static int synaptics_read_product_id(struct synaptics_ts_data *ts)
{
	...
	if (ret < 0) {
		TPDTM_DMESG
		    ("synaptics_read_product_id: failed for page select\n");
		return -1;
	}
	ret =
	    synaptics_rmi4_i2c_read_block(ts->client, F01_RMI_QUERY11, 8,
					  &(buf1[0x0]));
	ret =
	    synaptics_rmi4_i2c_read_block(ts->client, F01_RMI_QUERY_BASE + 19,
					  2, &(buf1[0x8]));
	if (ret < 0) {
		TPD_ERR
		    ("synaptics_read_product_id: failed to read product info\n");
		return -1;
	}
	return 0;
}
...
static int synaptics_enable_interrupt(struct synaptics_ts_data *ts, int enable)
{
	...
	if (ret < 0) {
		TPDTM_DMESG
		    ("synaptics_enable_interrupt: select page failed ret = %d\n",
		     ret);
		return -1;
	}
	if (enable) {
		...
		if (ret < 0) {
			TPDTM_DMESG
			    ("synaptics_enable_interrupt :clear interrupt bits failed\n");
			return -1;
		}
	} else {
		abs_status_int = 0x0;
	...
	ret =
	    synaptics_rmi4_i2c_write_byte(ts->client, F01_RMI_CTRL00 + 1,
					  abs_status_int);
	if (ret < 0) {
...
/* add by lifeng 2016/1/19 workarounds for the gestrue two interrupts end*/
	...
	if (version_is_s3508 == 2) {
		...
		ret =
		    i2c_smbus_read_i2c_block_data(ts->client, F51_CUSTOM_DATA11,
						  8, &(coordinate_buf[0]));
		ret =
		    i2c_smbus_read_i2c_block_data(ts->client,
						  F51_CUSTOM_DATA11 + 8, 8,
						  &(coordinate_buf[8]));
		ret =
		    i2c_smbus_read_i2c_block_data(ts->client,
						  F51_CUSTOM_DATA11 + 16, 8,
						  &(coordinate_buf[16]));
		ret =
		    i2c_smbus_read_i2c_block_data(ts->client,
						  F51_CUSTOM_DATA11 + 24, 1,
						  &(coordinate_buf[24]));
	}
/* add by lifeng 2016/1/19 workarounds for the gestrue two interrupts begin*/
 ...
 */
	...
	ret = synaptics_rmi4_i2c_write_byte(ts->client, 0xff, 0x0);
	Point_start.x =
	    (coordinate_buf[0] | (coordinate_buf[1] << 8)) * LCD_WIDTH /
	    (ts->max_x);
	Point_start.y =
	    (coordinate_buf[2] | (coordinate_buf[3] << 8)) * LCD_HEIGHT /
	    (ts->max_y);
	Point_end.x =
	    (coordinate_buf[4] | (coordinate_buf[5] << 8)) * LCD_WIDTH /
	    (ts->max_x);
	Point_end.y =
	    (coordinate_buf[6] | (coordinate_buf[7] << 8)) * LCD_HEIGHT /
	    (ts->max_y);
	Point_1st.x =
	    (coordinate_buf[8] | (coordinate_buf[9] << 8)) * LCD_WIDTH /
	    (ts->max_x);
	Point_1st.y =
	    (coordinate_buf[10] | (coordinate_buf[11] << 8)) * LCD_HEIGHT /
	    (ts->max_y);
	Point_2nd.x =
	    (coordinate_buf[12] | (coordinate_buf[13] << 8)) * LCD_WIDTH /
	    (ts->max_x);
	Point_2nd.y =
	    (coordinate_buf[14] | (coordinate_buf[15] << 8)) * LCD_HEIGHT /
	    (ts->max_y);
	Point_3rd.x =
	    (coordinate_buf[16] | (coordinate_buf[17] << 8)) * LCD_WIDTH /
	    (ts->max_x);
	Point_3rd.y =
	    (coordinate_buf[18] | (coordinate_buf[19] << 8)) * LCD_HEIGHT /
	    (ts->max_y);
	Point_4th.x =
	    (coordinate_buf[20] | (coordinate_buf[21] << 8)) * LCD_WIDTH /
	    (ts->max_x);
	Point_4th.y =
	    (coordinate_buf[22] | (coordinate_buf[23] << 8)) * LCD_HEIGHT /
	    (ts->max_y);
	clockwise = (coordinate_buf[24] & 0x10) ? 1 : (coordinate_buf[24] & 0x20) ? 0 : 2;	// 1--clockwise, 0--anticlockwise, not circle, report 2
}
...
static int set_tp_info(struct synaptics_ts_data *ts, uint8_t up_down)
{
	...
	ret = i2c_smbus_read_i2c_block_data(ts->client,
					    F51_CUSTOM_DATA_BASE,
					    sizeof(tp_infor), &(tp_infor[0]));
	ret = synaptics_rmi4_i2c_write_byte(ts->client, 0xff, 0x00);
	TPD_DEBUG("%s:tp info %d %d %d %d\n", __func__,
		  tp_infor[0] | tp_infor[1] << 8,
		  tp_infor[2] | tp_infor[3] << 8,
		  tp_infor[4] | tp_infor[5] << 8,
		  tp_infor[6] | tp_infor[7] << 8);
...
static void fp_detect(struct synaptics_ts_data *ts)
{
	...
	ret = i2c_smbus_write_byte_data(ts->client, 0xff, 0x00);
	ret = i2c_smbus_read_i2c_block_data(ts->client,
					    0x000A, 5, &(gesture_buffer[0]));

	gesture_sign = gesture_buffer[0];
	switch (gesture_sign) {
	case FINGER_DOWN:
		ts->fp_up_down = 1;
		sysfs_notify(&ts->dev->kobj, NULL, dev_attr_fp_irq.attr.name);
...
static void gesture_judge(struct synaptics_ts_data *ts)
{
	...
	if (version_is_s3508 == 2) {
		...
		TPD_DEBUG
		    ("Gesture Type[0x%x]=[0x%x],lpwg Swipe ID[0x4%x] = [0x%x]\n",
		     F12_2D_DATA04, gesture_buffer[0],
		     (F51_CUSTOM_DATA04 + 0x18), regswipe);
	ret = i2c_smbus_write_byte_data(ts->client, 0xff, 0x00);
	gesture_sign = gesture_buffer[0];
					 ...
					 loff_t * ppos)
{
	...
	ret = sprintf(page, "%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d\n",
		      gesture, Point_start.x, Point_start.y, Point_end.x,
		      Point_end.y, Point_1st.x, Point_1st.y, Point_2nd.x,
		      Point_2nd.y, Point_3rd.x, Point_3rd.y, Point_4th.x,
		      Point_4th.y, clockwise);
				  ...
				  size_t count, loff_t * ppos)
{
	...
	TPD_DEBUG("%s page=0x%x,address=0x%x,block=0x%x\n", __func__, page,
		  address, block);
	cnt +=
	    sprintf(&(buffer[cnt]), "page=0x%x,address=0x%x,block=0x%x\n", page,
		    address, block);
	...
	ret =
	    simple_read_from_buffer(user_buf, count, ppos, buffer,
				    strlen(buffer));
	return ret;
}
...
#ifdef SUPPORT_TP_SLEEP_MODE
static ssize_t tp_sleep_read_func(struct file *file, char __user * user_buf,
				  size_t count, loff_t * ppos)
{
	int ret = 0;
	char page[PAGESIZE];
...
static ssize_t tp_sleep_write_func(struct file *file, const char *buffer,
				   size_t count, loff_t * ppos)
{
	char buf[10] = { 0 };
	struct synaptics_ts_data *ts = ts_g;
	...
	default:
		TPDTM_DMESG
		    ("Please enter 0 or 1 to open or close the sleep function\n");
	}
	return count;
}
#endif
...
static int synaptics_read_register_map_page1(struct synaptics_ts_data *ts)
{
	...
	if (ret < 0) {
		TPD_ERR
		    ("synaptics_rmi4_i2c_write_byte failed for page select\n");
		return -1;
	}
	ret = synaptics_rmi4_i2c_read_block(ts->client, 0xE9, 4, &(buf[0x0]));
	F54_ANALOG_QUERY_BASE = buf[0];
...
static ssize_t tp_baseline_show(struct device_driver *ddri, char *buf)
{
	...
	ret =
	    i2c_smbus_write_byte_data(ts->client, F54_ANALOG_CONTROL_BASE + 20,
				      0x01);
	...
	if (version_is_s3508 != 2) {
		...
		ret = i2c_smbus_write_word_data(ts->client,
						F54_ANALOG_COMMAND_BASE, 0x04);
		ret = i2c_smbus_read_byte_data(ts->client, F54_ANALOG_CONTROL_BASE + 23);	/* Analog Control 1 */
		tmp_new = ret & 0xdf;
		i2c_smbus_write_byte_data(ts->client,
					  F54_ANALOG_CONTROL_BASE + 23,
					  tmp_new);
...
static ssize_t synaptics_rmi4_baseline_show_s3508(struct device *dev, char *buf,
						  bool savefile)
{
 ...
 TEST_WITH_CBC_s3508:
	...
	ret =
	    i2c_smbus_write_byte_data(ts->client, F54_ANALOG_CONTROL_BASE + 20,
				      0x01);
	ret =
	    i2c_smbus_read_byte_data(ts->client, F54_ANALOG_CONTROL_BASE + 23);
	tmp_old = ret & 0xff;
		...
		for (y = 0; y < RX_NUM; y++) {
			...
			if ((y < RX_NUM) && (x < TX_NUM)) {
				...
				if (((baseline_data + 60) <
					...
					*(baseline_data_test + count * 2 +
					  1))) {
					if ((x == (TX_NUM - 1) && (y != RX_NUM - 1 || y != RX_NUM - 2)) || (x != (TX_NUM - 1) && (y == RX_NUM - 1 || y == RX_NUM - 2)))	//the last tx and rx last two line for touchkey,others no need take care
					{
						count++;
						continue;
					}
	...
	ret =
	    i2c_smbus_write_word_data(ts->client, F54_ANALOG_DATA_BASE + 1,
				      0x0);
	ret = i2c_smbus_write_byte_data(ts->client, F54_ANALOG_COMMAND_BASE, 0x01);	//get report
	checkCMD(10);
	tx_datal =
	    i2c_smbus_read_i2c_block_data(ts->client, F54_ANALOG_DATA_BASE + 3,
					  7, buffer);
	if (ts->support_1080x2160_tp) {
	...
	for (y = 0; y < RX_NUM; y++) {
		...
		if (baseline_data > 100) {
			error_count++;
			TPD_ERR
			    ("step 4:error_count[%d] baseline_data%d[0x%x] ERROR!\n",
		...
		for (y = 0; y < RX_NUM; y++) {
			...
			if ((x < TX_NUM - 1) && (y < RX_NUM - 2) && (baseline_data > 20)) {	/*red zone, test capacitance,if half num large than 20, fail */
				if (++err_RT251 >
					...
					error_count++;
					TPD_ERR
					    ("Step 5 : Check RT251 for random touch event error, capacitance, err_RT251 is %d\n",
					     err_RT251);
					goto END;
				}
			}
			...
			if ((x != TX_NUM - 1) && (y == RX_NUM - 1) && baseline_data > 500) {	/*blue zone, test self capaccitance, if half num large than 500, fail */
				if (++err_RT251_self > (TX_NUM - 1) / 2) {
					error_count++;
					TPD_ERR
					    ("Step 5 : Check RT251 for random touch event error, self capacitance, err_RT251_self is %d\n",
					     err_RT251_self);
					goto END;
				}
			}
	...
	for (y = 0; y < RX_NUM + TX_NUM - 3; y++) {
		...
		if (unsigned_baseline_data < 10000) {
			error_count++;
			TPD_ERR
			    ("Step 6 : Check RT252 for random touch event, error_line is y =%d,data = %hu\n",
		...
		for (y = 0; y < RX_NUM; y++) {
			...
			if (baseline_data > 20) {
				if (++err_RT253 > (TX_NUM * RX_NUM) / 2) {
					error_count++;
					TPD_ERR
					    ("Step 7 : Check RT253 for random touch event, self capacitance, err_RT253 is %d\n",
					     err_RT253);
					goto END;
				}
			}
 ...
 END:
	...
	num_read_chars += snprintf(&(buf[num_read_chars]), 128,
				   ...
				   TP_FW);
	num_read_chars +=
	    snprintf(&(buf[num_read_chars]), 128, "left:=%d,right:%d\n",
		     left_ramdata, right_ramdata);
	num_read_chars +=
	    snprintf(&(buf[num_read_chars]), 128, "%d error(s). %s\n",
		     error_count, error_count ? "" : "All test passed.");
	return num_read_chars;
}

static ssize_t synaptics_rmi4_baseline_show_s3706(struct device *dev, char *buf,
						  bool savefile)
{
	ssize_t num_read_chars = 0;
#if TP_TEST_ENABLE
	...
	//msm_cpuidle_set_sleep_disable(true);
	ret = synaptics_rmi4_i2c_write_byte(ts->client, 0xff, 0x0);
	synaptics_rmi4_i2c_read_block(ts->client, F34_FLASH_CTRL00, 8, buf);
 ...
 READDATA_AGAIN:
	...
	if (savefile) {
		...
		snprintf(data_buf, 40,
			 ...
			 rtc_now_time.tm_min, rtc_now_time.tm_sec);
 ...
 TEST_WITH_CBC_s3508:
	...
	if (enable_cbc) {
		...
		if (ret < 0) {
			TPD_ERR
			    ("read_baseline: i2c_smbus_write_byte_data failed\n");
			goto END;
		}
		ret = i2c_smbus_write_byte_data(ts->client,
						F54_ANALOG_CONTROL_BASE + 20,
						0x01);
		ret =
		    i2c_smbus_read_byte_data(ts->client,
					     F54_ANALOG_CONTROL_BASE + 23);
		...
		for (y = 0; y < RX_NUM; y++) {
			...
			if ((y < RX_NUM) && (x < TX_NUM)) {
				...
				if (((baseline_data + 60) <
					...
					TPD_ERR
					    ("failed,RX_NU:%d,TX_NU:%d,baslin_da is%d\n",
					     y, x, baseline_data);
					TPD_ERR
					    ("failed,TPK_arrlimit[%d*2]=%d,TPK_arrlimit[%d*2+1]=%d\n",
					     count,
					     *(baseline_data_test + count * 2),
					     count,
					     *(baseline_data_test + count * 2 +
	...
	ret = i2c_smbus_write_byte_data(ts->client, F54_ANALOG_DATA_BASE, 0x96);	/*select report type 0x96 */
	ret = i2c_smbus_write_word_data(ts->client, F54_ANALOG_DATA_BASE + 1, 0x00);	/*set fifo 00 */
	ret = i2c_smbus_write_byte_data(ts->client, F54_ANALOG_COMMAND_BASE, 0x01);	/*get report */
	checkCMD(10);
	/*all channel 16+33 */
...
static int synaptics_rmi4_reset_device(struct i2c_client *client, bool rebuild)
{
	int retval;
...
static int synatpitcs_fw_update(struct device *dev, bool force)
{
	...
	if (ret < 0) {
		...
		if (ret < 0) {
			TPD_ERR
			    ("FW update failed twice, quit updating process!\n");
			return ret;
		}
	}
	release_firmware(fw);
	...
	if (ret < 0) {
		...
		temp =
		    temp + sprintf(&buf[temp],
				   "Request failed,Check the path %d", temp);
		return temp;
	}
...
int tp_single_tap_en(struct synaptics_ts_data *ts, bool enable)
{
	...
	if (ret < 0)
		return ret;
	if (enable) {
		ret = synaptics_rmi4_i2c_write_byte(ts->client, 0x22, 0x01);
		ret = synaptics_rmi4_i2c_write_byte(ts->client, 0x23, 0x1e);
		ret = synaptics_rmi4_i2c_write_byte(ts->client, 0x24, 0x1e);
		ret = synaptics_rmi4_i2c_write_byte(ts->client, 0x25, 0x32);
		ret = i2c_smbus_write_word_data(ts->client, F54_ANALOG_COMMAND_BASE, 0x04);	// force update
		TPD_DEBUG("%s: force update %x\n",
			  __func__, F54_ANALOG_COMMAND_BASE);
	} else
		ret = synaptics_rmi4_i2c_write_byte(ts->client, 0x22, 0x00);
...
static ssize_t tp_reset_write_func(struct file *file, const char *buffer,
				   size_t count, loff_t * ppos)
{
...
static ssize_t changer_write_func(struct file *file, const char __user * buffer,
				  size_t count, loff_t * ppos)
{
	struct synaptics_ts_data *ts = ts_g;
	int ret = 0;
...
static int tp_baseline_get(struct synaptics_ts_data *ts, bool flag)
{
	...
	ret =
	    synaptics_rmi4_i2c_read_block(ts->client, F54_ANALOG_DATA_BASE + 3,
					...
					size_t count, loff_t * ppos)
{
	...
	if (ret == 0) {
		...
		if (0 == ts->gesture_enable)
			queue_delayed_work(get_base_report, &ts->base_work,
...
#ifdef ENABLE_TPEDGE_LIMIT
static ssize_t limit_enable_read(struct file *file, char __user * user_buf,
				 size_t count, loff_t * ppos)
{
	ssize_t ret = 0;
	char page[PAGESIZE];
...
static ssize_t limit_enable_write(struct file *file, const char __user * buffer,
				  size_t count, loff_t * ppos)
{
	int ret;
	char buf[8] = { 0 };
	...
	TPD_ERR("%s_proc limit_enable =%d,mode:0x%x,grip:0x%x!\n",
		__func__, limit_enable, limit_mode, F51_GRIP_CONFIGURATION);
	if (limit_mode) {
...
#endif
#ifdef SUPPORT_TP_TOUCHKEY
static ssize_t key_switch_read_func(struct file *file, char __user * user_buf,
				    size_t count, loff_t * ppos)
{
	int ret = 0;
	char page[PAGESIZE];
...
static ssize_t key_disable_read_func(struct file *file, char __user * user_buf,
				     size_t count, loff_t * ppos)
{
	int ret = 0;
	char page[PAGESIZE];
	...
	ret =
	    sprintf(page, "cmd:enable,disable\nkey_back:%s key_appselect:%s\n",
		    key_back_disable ? "disable" : "enable",
		    key_appselect_disable ? "disable" : "enable");
	ret =
	    simple_read_from_buffer(user_buf, count, ppos, page, strlen(page));
...
static int init_synaptics_proc(void)
{
...
#ifdef SUPPORT_GLOVES_MODE
	prEntry_tmp =
	    proc_create("glove_mode_enable", 0666, prEntry_tp,
			&glove_mode_enable_proc_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
...
#ifdef SUPPORT_TP_SLEEP_MODE
	prEntry_tmp =
	    proc_create("sleep_mode_enable", 0666, prEntry_tp,
			&sleep_mode_enable_proc_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp =
	    proc_create("tp_reset", 0666, prEntry_tp, &tp_reset_proc_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
...
#ifdef ENABLE_TPEDGE_LIMIT
	prEntry_tmp =
	    proc_create("tpedge_limit_enable", 0666, prEntry_tp,
			&proc_limit_enable);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp =
	    proc_create("baseline_test", 0666, prEntry_tp,
			&tp_baseline_test_proc_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_ERR("Couldn't create baseline_test\n");
	}
	prEntry_tmp =
	    proc_create("i2c_device_test", 0666, prEntry_tp,
			&i2c_device_test_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp =
	    proc_create("radd", 0777, prEntry_tp, &base_register_address);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_ERR("Couldn't create radd\n");
	}
	prEntry_tmp =
	    proc_create("vendor_id", 0444, prEntry_tp, &vendor_id_proc_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_ERR("Couldn't create vendor_id\n");
	}
	prEntry_tmp =
	    proc_create("changer_connet", 0666, prEntry_tp, &changer_ops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp =
	    proc_create("touch_press", 0666, prEntry_tp, &touch_press_status);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
...
#ifdef SUPPORT_TP_TOUCHKEY
	prEntry_tmp =
	    proc_create("key_switch", 0666, prEntry_tp, &key_switch_proc_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp =
	    proc_create("key_disable", 0666, prEntry_tp,
			&key_disable_proc_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
...
static int checkFlashState(struct i2c_client *client)
{
	...
	while ((ret != 0x80) && (count < 8)) {
		msleep(3);	//wait 3ms
		ret =
		    synaptics_rmi4_i2c_read_byte(client,
...
static int synaptics_fw_check(struct synaptics_ts_data *ts)
{
	...
	bootloader_mode =
	    synaptics_rmi4_i2c_read_byte(ts->client, F01_RMI_DATA_BASE);
	bootloader_mode = bootloader_mode & 0xff;
	bootloader_mode = bootloader_mode & 0x40;
	TPD_DEBUG
	    ("afte fw update,program memory self-check bootloader_mode = 0x%x\n",
	     bootloader_mode);
...
static int synaptics_rmi4_query_firmware_id_s3706(struct i2c_client *client,
						  unsigned int
						  *firmware_id_s3706,
						  unsigned char
						  *conifg_id_s3706)
			       ...
			       uint32_t data_len, bool force)
{
	...
	if (!client)
		return -1;
	if (!strncmp(ts->manu_name, "S3706", 5)) {
		synaptics_rmi4_query_firmware_id_s3706(client,
						       &CURRENT_FIRMWARE_ID,
						       CURRENT_CONFIG_ID);
		TPD_ERR("FW_ID:%d--CONFIG_ID %s FW_NAME:%s\n",
			CURRENT_FIRMWARE_ID, CURRENT_CONFIG_ID, ts->fw_name);
		...
		ret =
		    synaptics_rmi4_i2c_read_block(client, F34_FLASH_CTRL00, 4,
						  buf);
		CURRENT_FIRMWARE_ID =
		    (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
		FIRMWARE_ID =
		    (Config_Data[0] << 24) | (Config_Data[1] << 16) |
		    (Config_Data[2] << 8) | Config_Data[3];
		if (1 == check_version)
			TPD_ERR
			    ("15801CURRENT_FW_ID:%x----, FW_ID:%x----,FW_NAME:%s\n",
			     CURRENT_FIRMWARE_ID, FIRMWARE_ID, ts->fw_name);
		else
			TPD_ERR
			    ("15801CURRENT_FW_ID:%xvB----, FW_ID:%xvB----,FW_NAME:%s\n",
			     CURRENT_FIRMWARE_ID, FIRMWARE_ID, ts->fw_name);
		...
		if ((header.firmware_size + header.config_size + 0x100) >
		    data_len) {
			TPDTM_DMESG
			    ("firmware_size + config_size + 0x100 > data_len data_len = %d \n",
			     data_len);
			return -1;
		}
		...
		ret =
		    i2c_smbus_read_i2c_block_data(client, F34_FLASH_CTRL00, 4,
						  buf);
		CURRENT_FIRMWARE_ID =
		    (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
		FIRMWARE_ID =
		...
		ret =
		    i2c_smbus_write_i2c_block_data(client,
						   ...
						   &(bootloder_id[0x0]));
		TPD_DEBUG
		    ("Write bootloader id SynaF34_FlashControl is 0x00%x ret is %d\n",
		...
		if (ret > 0) {
			TPD_ERR
			    ("Get in prog:The status(Image) of flashstate is %x\n",
			     ret);
			return -1;
		}
		...
		ret =
		    i2c_smbus_write_byte_data(client, F01_RMI_CTRL_BASE,
					      ret & 0x04);
		/********************get into prog end************/
		ret =
		    i2c_smbus_write_i2c_block_data(client,
		...
		if (ret != 0x00)
			msleep(2000);
		ret =
		    i2c_smbus_read_byte_data(client, SynaF34_FlashControl + 1);
		TPDTM_DMESG("The status(erase) is %x\n", ret);
		TPD_ERR
		    ("15811update-----------------update------------------update!\n");
		TPD_DEBUG("cnt %d\n", firmware);
		for (j = 0; j < firmware; j++) {
			...
			i2c_smbus_write_byte_data(client, SynaF34_FlashControl,
						  0x02);
			ret = checkFlashState(client);
			if (ret > 0) {
				TPD_ERR
				    ("Firmware:The status(Image) of flash data3 is %x,time =%d\n",
				     ret, j);
				return -1;
			}
		}
		...
		for (j = 0; j < configuration; j++) {
			...
			if (ret > 0) {
				TPD_ERR
				    ("Configuration:The status(Image) of flash data3 is %x,time =%d\n",
				     ret, j);
				return -1;
			}
		}
		...
		synaptics_rmi4_i2c_write_byte(client, SynaF34_FlashControl,
					      0x0F);
		msleep(10);
		TPD_DEBUG("attn step 4\n");
		ret = checkFlashState(client);
		...
		ret = i2c_smbus_read_byte_data(client, SynaF34_FlashControl);
		TPDTM_DMESG
		    ("going to flash firmware area synaF34_FlashControl %d\n",
		     ret);
...
static int synaptics_parse_dts(struct device *dev, struct synaptics_ts_data *ts)
{
	...
	np = dev->of_node;
	ts->irq_gpio =
	    of_get_named_gpio_flags(np, "synaptics,irq-gpio", 0,
				    &(ts->irq_flags));
	if (ts->irq_gpio < 0) {
	...
	TPD_ERR("%s ts->support_hw_poweroff =%d\n", __func__,
		ts->support_hw_poweroff);

	ts->enable2v8_gpio =
	    of_get_named_gpio(np, "synaptics,enable2v8-gpio", 0);
	if (ts->enable2v8_gpio < 0) {
	...
	rc = of_property_read_u32_array(np, "synaptics,button-map", button_map,
					3);
	if (rc) {
	...
	rc = of_property_read_u32_array(np, "synaptics,tx-rx-num", tx_rx_num,
					2);
	if (rc) {
	...
	if (rc < 0) {
		...
		rc = regulator_set_voltage(ts->vdd_2v8,
					   ts->regulator_avdd_vmin,
					   ts->regulator_avdd_vmax);
		if (rc < 0)
			TPD_ERR("%s:Failed to set avdd\n", __func__);
	}
	...
	if (ts->enable2v8_gpio > 0) {
		if (gpio_is_valid(ts->enable2v8_gpio)) {
			...
			if (rc)
				TPD_ERR
				    ("unable to request enable2v8_gpio [%d]\n",
				     ts->enable2v8_gpio);
			    ...
			    struct kobj_attribute *attr, char *buf)
{
	...
	len = sprintf(buf,
		      ...
		      ":%d:%d:%d:%d" "\n", VK_KEY_X, VK_CENTER_Y, VK_WIDTH,
		      VK_HIGHT, VK_KEY_X * 3, VK_CENTER_Y, VK_WIDTH, VK_HIGHT,
			      ...
			      const struct i2c_device_id *id)
{
	...
	if (driver_create_file
	    (&tpd_i2c_driver.driver, &driver_attr_tp_baseline_image_with_cbc)) {
		TPDTM_DMESG("driver_create_file failt\n");
		goto exit_init_failed;
	}
	if (driver_create_file
	    (&tpd_i2c_driver.driver, &driver_attr_tp_baseline_image)) {
		TPDTM_DMESG("driver_create_file failt\n");
		goto exit_init_failed;
	}
	if (driver_create_file
	    (&tpd_i2c_driver.driver, &driver_attr_tp_delta_image)) {
		TPDTM_DMESG("driver_create_file failt\n");
		goto exit_init_failed;
	}
	if (ts->project_version == 0x03) {
...
static int synaptics_ts_remove(struct i2c_client *client)
{
	...
	if (msm_drm_unregister_client(&ts->msm_drm_notif))
		dev_err(&client->dev,
			"Error occurred while unregistering msm_drm_notifier.\n");
#endif
